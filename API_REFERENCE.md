...
- **`healthBeacon`** (default `true`): when ingest is rejected at the domain gate (HTTP 403), or TraceLog reports that the project id doesn't resolve (HTTP 404), emit a diagnostic beacon so the dashboard can tell you your snippet is alive but events are blocked or misconfigured. The 404 beacon is sent only when the response is identifiably TraceLog's, so a 404 from a misconfigured CNAME target is never reported as a bad project id. Throttled to at most one per 10 minutes per browser, per reason (persisted in localStorage, shared across pages and tabs). Diagnostic only — never carries analytics data. Set `false` to opt out.
