...
1. **Feature Development**: Create branch → Implement → Test locally
2. **Testing**: Run `npm run test` (unit + integration + E2E) - must pass 100%
3. **Quality Check**: Run `npm run check` before commit (lint + format)
4. **Build Verification**: Run `npm run build:all` to ensure no build errors
5. **Type Check**: Run `npm run type-check` to verify no type errors
6. **Submit PR**: All checks must pass in CI (security, quality, build, tests)
7. **Release**: Merge to `main` → GitHub Actions handles versioning and npm publish
