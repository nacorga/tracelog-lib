...
// 6. With Event Transformers (v1.1.0+) - Runtime methods only
...
tracelog.setTransformer('beforeSend', (event) => ({
  ...event,
  auth_token: authToken  // Use pre-fetched data (sync)
}));
...
### Event Transformers (Runtime Methods, v1.1.0+)

**Runtime Transformer Methods** - set dynamically after init:
- `setTransformer(hook, fn)` - Add or update transformer
- `removeTransformer(hook)` - Remove transformer
...
**Critical Rules:**
- **Synchronous only** - No `async`/`await` (validated, throws error)
- **Fast execution** - Target <10ms, warnings if >50ms
- **Error safe** - Automatic error catching, original event as fallback
- **Return `null`** - Filters event from batch or cancels batch send
...
// Update transformer dynamically (e.g., on auth refresh)
setInterval(() => {
  tracelog.setTransformer('beforeBatch', (queue) => ({
    ...queue,
...
**See:** [README.md](./README.md#event-transformers), [API_REFERENCE.md](./API_REFERENCE.md#event-transformers-runtime-methods), [BEST_PRACTICES.md](./BEST_PRACTICES.md#event-transformers), [SECURITY.md](./SECURITY.md#4-event-transformer-security)
...
### Documentation
- ❌ **NEVER touch CHANGELOG.md** - Auto-generated by CI/CD GitHub workflow
- ✅ DO update README.md, API_REFERENCE.md, SECURITY.md, BEST_PRACTICES.md when code changes
- ✅ DO update component docs (src/handlers/README.md, src/managers/README.md) when relevant
...
- [CHANGELOG.md](./CHANGELOG.md) - **DO NOT EDIT** - Auto-generated by CI/CD from conventional commits
...
**IMPORTANT**: CHANGELOG.md is automatically generated by GitHub Actions workflow based on conventional commit messages. NEVER manually edit this file.
