...
describe('API Integration - Init Flow', () => {
  ...
  describe('Successful Initialization', () => {
    it('should initialize successfully with valid config', async () => {
      const config: AppConfig = { id: SpecialProjectId.Skip };
  ...
  describe('Config Validation - Strict Mode', () => {
    it('should throw error for missing project id', async () => {
      ...
      await expect(TraceLog.init({})).rejects.toThrow();
    });

    it('should throw error for empty project id', async () => {
      await expect(TraceLog.init({ id: '' })).rejects.toThrow();
    });

    it('should throw error for whitespace-only project id', async () => {
      await expect(TraceLog.init({ id: '   ' })).rejects.toThrow();
    });

    it('should reject negative sessionTimeout (strict validation)', async () => {
      await expect(TraceLog.init({ id: SpecialProjectId.Skip, sessionTimeout: -1000 })).rejects.toThrow();
    });

    it('should reject errorSampling > 1 (strict validation)', async () => {
      await expect(TraceLog.init({ id: SpecialProjectId.Skip, errorSampling: 2.5 })).rejects.toThrow();
    });

    it('should reject errorSampling < 0 (strict validation)', async () => {
      await expect(TraceLog.init({ id: SpecialProjectId.Skip, errorSampling: -0.5 })).rejects.toThrow();
    });
  });
  ...
  describe('Valid Config Boundaries', () => {
    ...
    it('should accept valid sessionTimeout', async () => {
      await TraceLog.init({ id: SpecialProjectId.Skip, sessionTimeout: 900000 });
      expect(TraceLog.isInitialized()).toBe(true);
    });

    it('should accept errorSampling = 0 (disable error tracking)', async () => {
      await TraceLog.init({ id: SpecialProjectId.Skip, errorSampling: 0 });
      expect(TraceLog.isInitialized()).toBe(true);
    });

    it('should accept errorSampling = 1 (track all errors)', async () => {
      await TraceLog.init({ id: SpecialProjectId.Skip, errorSampling: 1 });
      expect(TraceLog.isInitialized()).toBe(true);
    });

    it('should accept errorSampling = 0.5 (track 50%)', async () => {
      await TraceLog.init({ id: SpecialProjectId.Skip, errorSampling: 0.5 });
      expect(TraceLog.isInitialized()).toBe(true);
    });
  });
