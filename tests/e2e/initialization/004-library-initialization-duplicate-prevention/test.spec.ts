...
test.describe('Library Initialization - Duplicate Prevention', () => {
  // Constants
  ...
  test('should handle multiple consecutive init() calls with same configuration gracefully', async ({ page }) => {
    ...
    try {
      ...
      const criticalErrors = monitor.traceLogErrors.filter((error: string) => 
  ...
  test('should handle multiple init() calls with different configurations properly', async ({ page }) => {
    // Setup console monitoring
    const monitor = TestHelpers.createConsoleMonitor(page);
    ...
    try {
      ...
      // Storage should not have excessive growth
      const finalStorageKeys = await TestHelpers.getTraceLogStorageKeys(page);
  ...
  test('should not create duplicate event handlers during multiple initializations', async ({ page }) => {
    // Setup console monitoring
    const monitor = TestHelpers.createConsoleMonitor(page);

    try {
      // Initial setup - first initialization
      const firstInitResult = await TestHelpers.initializeTraceLog(page, DEFAULT_TEST_CONFIG);
      ...
      // Should not have exponential growth of stored data
      expect(finalEvents).toBeLessThan(initialEvents * 3); // Reasonable upper bound

      // Verify no handler-related errors
      const handlerErrors = monitor.traceLogErrors.filter((error: string) => 
        error.toLowerCase().includes('handler') || 
  ...
  test('should maintain existing tracking functionality after multiple initializations', async ({ page }) => {
    // Setup console monitoring
    const monitor = TestHelpers.createConsoleMonitor(page);
    ...
    try {
      ...
      for (let i = 0; i < configs.length; i++) {
        ...
        // Re-initialize (should succeed gracefully)
      ...
      const trackingErrors = monitor.traceLogErrors.filter((error: string) => 
        error.toLowerCase().includes('track') && 
  ...
  test('should validate memory management during repeated re-initialization attempts', async ({ page }) => {
    // Setup console monitoring
    const monitor = TestHelpers.createConsoleMonitor(page);
    ...
    try {
      ...
      const memoryErrors = monitor.traceLogErrors.filter((error: string) => 
        error.toLowerCase().includes('memory') ||
  ...
  test('should ensure consistent behavior across all re-initialization attempts', async ({ page }) => {
    // Setup console monitoring
    const monitor = TestHelpers.createConsoleMonitor(page);

    try {
      // Track behavior consistency across multiple init calls
      const results: any[] = [];
      const configs = [DEFAULT_TEST_CONFIG, ALTERNATE_CONFIG, COMPLEX_CONFIG, DEFAULT_TEST_CONFIG];

      // First initialization (baseline)
      const baselineResult = await TestHelpers.initializeTraceLog(page, DEFAULT_TEST_CONFIG);
      const baselineValidated = TestAssertions.verifyInitializationResult(baselineResult);
      expect(baselineValidated.success).toBe(true);
      
      results.push({
        attempt: 0,
        config: DEFAULT_TEST_CONFIG.id,
        success: baselineValidated.success,
        error: baselineResult.error,
        isInitialized: await TestHelpers.isTraceLogInitialized(page)
      });

      // Multiple re-initialization attempts with different configs
      for (let i = 0; i < configs.length; i++) {
        await TestHelpers.waitForTimeout(page, 300);
        
        const attemptResult = await TestHelpers.initializeTraceLog(page, configs[i]);
        const attemptValidated = TestAssertions.verifyInitializationResult(attemptResult);
        
        results.push({
          attempt: i + 1,
          config: configs[i].id,
          success: attemptValidated.success,
      ...
      // According to api.ts, all re-initializations should succeed consistently
      const allSuccessful = results.every(r => r.success);
      const consistentlyInitialized = results.every(r => r.isInitialized);

      // All subsequent calls should succeed (as per api.ts line 26-28)
      expect(allSuccessful).toBe(true);

      // TraceLog should always remain initialized
      expect(consistentlyInitialized).toBe(true);

      // Final verification of overall state
      const finalState = await page.evaluate(() => ({
        isInitialized: (window as any).TraceLog.isInitialized(),
        canTrackEvents: true
      }));

      // Test final event tracking capability
      try {
        const finalEventResult = await TestHelpers.testCustomEvent(page, 'consistency_test_final', {
          totalAttempts: results.length,
          successfulAttempts: results.filter(r => r.success).length
        });
        finalState.canTrackEvents = TestAssertions.verifyInitializationResult(finalEventResult).success;
      } catch (e) {
        finalState.canTrackEvents = false;
      }

      expect(finalState.isInitialized).toBe(true);
      expect(finalState.canTrackEvents).toBe(true);
    } finally {
      monitor.cleanup();
    }
  });

  test('should properly handle rapid consecutive initialization attempts', async ({ page }) => {
    // Setup console monitoring
    const monitor = TestHelpers.createConsoleMonitor(page);
    ...
    try {
      ...
      for (let i = 0; i < 5; i++) {
        ...
        rapidInitPromises.push(
          page.evaluate(async (cfg) => {
            try {
              return await (window as any).initializeTraceLog(cfg);
            } catch (error: any) {
      ...
      const allHaveResults = rapidResults.every(r => r.hasOwnProperty('success'));
      ...
      // Note: "initialization is already in progress" errors are expected and acceptable
      const unexpectedRaceErrors = monitor.traceLogErrors.filter((error: string) => 
