...
export class TestHelpers {
   ...
   */
  static async navigateAndWaitForReady(page: Page, url: string, statusSelector?: string) {
    await page.goto(url);
  ...
  static async waitForReadyStatus(page: Page) {
    await page.waitForSelector('[data-testid="init-status"]:has-text("Status: Ready for testing")');
  }

  static async waitForValidationReadyStatus(page: Page) {
  ...
  static async verifyTraceLogAvailability(page: Page): Promise<boolean> {
    return await page.evaluate(() => {
      return typeof (window as any).TraceLog !== 'undefined' && 
  ...
  static async initializeTraceLog(page: Page, config = { id: 'test' }) {
   ...
   */
  static async verifyInitializationStatus(page: Page, expectedStatus: string) {
    await page.waitForSelector(`[data-testid="init-status"]:has-text("${expectedStatus}")`);
  }

  static async verifyValidationStatus(page: Page, expectedStatus: string) {
   ...
   */
  static async triggerClickEvent(page: Page, selector: string = 'h1[data-testid="title"]') {
    await page.click(selector);
    await page.waitForTimeout(500);
  }

  static async triggerScrollEvent(page: Page) {
  ...
  static async testCustomEvent(page: Page, eventName: string = 'test_event', metadata: any = { test: true }) {
    return await page.evaluate(({ eventName, metadata }) => {
      try {
   ...
   */
  static async createIsolatedContext(browser: Browser) {
   ...
   * Timeout utilities
   */
  static async waitForTimeout(page: Page, timeout: number = 1000) {
...
export class TestAssertions {
  static verifyInitializationResult(result: any) {
