...
- **Ingestion receipt** — parses the server's account of the batch (`accepted` / `duplicates` / `filtered` / `dropped`, plus `outcome`, `coverage`, and an optional refusal `reason`) off both a 2xx body and a rejection envelope, and hands it to the send callbacks. It travels as a `send()` return value rather than an instance field: `sendEventsQueue()` has no in-flight guard, so a shared field would let two overlapping sends misattribute each other's counts. On a rejection the body must first prove a TraceLog host wrote it (a `statusCode` echoing the HTTP status) — an unvouched receipt is a false claim about the merchant's data, not a missing log detail. `sendBeacon` batches expose no response, so they yield no receipt
