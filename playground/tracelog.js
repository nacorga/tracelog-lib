...
const De = (r, e) => e ? `[TraceLog] ${r}: ${e instanceof Error ? e.message : "Unknown error"}` : `[TraceLog] ${r}`, o = (r, e, t) => {
  const { error: s, data: n, showToClient: i } = t ?? {}, a = s ? De(e, s) : `[TraceLog] ${e}`, c = r === "error" ? "error" : r === "warn" ? "warn" : "log";
  i && (n !== void 0 ? console[c](a, n) : console[c](a));
};
let W, pe;
const ke = () => {
  ...
  try {
    ...
    ke();
    const e = window.innerWidth, t = W?.matches ?? !1, s = pe?.matches ?? !1, n = "ontouchstart" in window || navigator.maxTouchPoints > 0, i = navigator.userAgent.toLowerCase(), a = /mobile|android|iphone|ipod|blackberry|iemobile|opera mini/.test(i), c = /tablet|ipad|android(?!.*mobile)/.test(i);
    return e <= 767 || a && n ? _.Mobile : e >= 768 && e <= 1024 || c || t && s && n ? _.Tablet : _.Desktop;
  } catch (r) {
    return o("warn", "Device detection failed, defaulting to desktop", { error: r }), _.Desktop;
  }
}, T = "tlog", ce = `${T}:qa_mode`, He = `${T}:uid`, xe = (r) => r ? `${T}:${r}:queue` : `${T}:queue`, Ve = (r) => r ? `${T}:${r}:session` : `${T}:session`, Fe = (r) => r ? `${T}:${r}:broadcast` : `${T}:broadcast`, _e = {
  LCP: 4e3,
  FCP: 1800,
  CLS: 0.25,
  INP: 200,
  ...
  /:\/\/[^:/]+:([^@]+)@/gi
], le = 500, ue = 5e3, k = 50, Qe = k * 2, de = "tlog_mode", ze = "qa", $e = () => {
  ...
  if (r.integrations?.tracelog?.projectId) {
    ...
    const n = r.integrations.tracelog.projectId, i = s.slice(-2).join("."), a = `https://${n}.${i}`;
    if (!fe(a))
      throw new Error("Invalid URL");
    return a;
  }
  if (r.integrations?.custom?.apiUrl) {
    const e = r.integrations.custom.apiUrl, t = r.integrations?.custom?.allowHttp ?? !1;
    if (!fe(e, t))
      throw new Error("Invalid URL");
    return e;
  }
  ...
  try {
    ...
    const i = [];
    return e.forEach((c) => {
      s.has(c) && (s.delete(c), n = !0, i.push(c));
  ...
  if (r.scrollContainerSelectors !== void 0 && qe(r.scrollContainerSelectors), r.integrations && Ze(r.integrations), r.sensitiveQueryParams !== void 0) {
  ...
  if (r) {
    if (r.tracelog && (!r.tracelog.projectId || typeof r.tracelog.projectId != "string" || r.tracelog.projectId.trim() === ""))
      throw new y(m.INVALID_TRACELOG_PROJECT_ID, "config");
    if (r.custom) {
      if (!r.custom.apiUrl || typeof r.custom.apiUrl != "string" || r.custom.apiUrl.trim() === "")
        throw new y(m.INVALID_CUSTOM_API_URL, "config");
      const e = r.custom.apiUrl.trim();
      if (!e.startsWith("http://") && !e.startsWith("https://"))
        throw new y('Custom API URL must start with "http://" or "https://"', "config");
    ...
    if (r.googleAnalytics) {
      if (!r.googleAnalytics.measurementId || typeof r.googleAnalytics.measurementId != "string" || r.googleAnalytics.measurementId.trim() === "")
        throw new y(m.INVALID_GOOGLE_ANALYTICS_ID, "config");
      if (!r.googleAnalytics.measurementId.trim().match(/^(G-|UA-)/))
        throw new y('Google Analytics measurement ID must start with "G-" or "UA-"', "config");
    }
  }
}, Je = (r) => (Ye(r), {
  ...r,
...
class ot extends f {
  ...
  constructor(e, t = null, s = null) {
    super(), this.googleAnalytics = t, this.dataSender = new at(e), this.emitter = s;
  }
  ...
  track({
    ...
    if (!e) {
      o("warn", "Event type is required");
      return;
    }
    const p = e === d.SESSION_START || e === d.SESSION_END;
    if (!p && !this.checkRateLimit()) {
      o("warn", "Rate limit exceeded, event dropped", {
        data: {
          type: e,
    ...
    const I = e, Le = I === d.SESSION_START;
    d.SESSION_END;
    const Re = t || this.get("pageUrl"), Q = this.buildEventPayload({
      type: I,
      page_url: Re,
      from_page_url: s,
      scroll_data: n,
    ...
    if (!(!p && !this.shouldSample())) {
      if (Le) {
        ...
        if (!ae) {
          o("warn", "Session start event ignored: missing sessionId");
          return;
        }
      ...
      if (!this.isDuplicateEvent(Q)) {
        if (this.get("mode") === R.QA && I === d.CUSTOM && a) {
  ...
  flushPendingEvents() {
    if (this.pendingEventsBuffer.length === 0)
      return;
    if (!this.get("sessionId")) {
      o("warn", "Cannot flush pending events: session not initialized");
      return;
    }
  ...
  checkRateLimit() {
    const e = Date.now();
    return e - this.rateLimitWindowStart > 1e3 && (this.rateLimitCounter = 0, this.rateLimitWindowStart = e), this.rateLimitCounter >= 100 ? !1 : (this.rateLimitCounter++, !0);
  }
...
class lt extends f {
  ...
  initCrossTabSync() {
    ...
    this.broadcastChannel = new BroadcastChannel(Fe(e)), this.broadcastChannel.onmessage = (t) => {
...
class ut extends f {
  ...
  async startTracking() {
    ...
    try {
      this.sessionManager = new lt(this.storageManager, this.eventManager, t), await this.sessionManager.startTracking(), this.eventManager.flushPendingEvents();
    } catch (s) {
...
class ht extends f {
  ...
  calculateClickCoordinates(e, t) {
    const s = t.getBoundingClientRect(), n = e.clientX, i = e.clientY, a = s.width > 0 ? this.clamp((n - s.left) / s.width) : 0, c = s.height > 0 ? this.clamp((i - s.top) / s.height) : 0;
  ...
  generateClickData(e, t, s) {
    const { x: n, y: i, relativeX: a, relativeY: c } = s, l = this.getRelevantText(e, t), u = this.extractElementAttributes(t);
    return {
      x: n,
      y: i,
      relativeX: a,
...
class ft extends f {
  ...
  calculateScrollData(e) {
    ...
    if (Math.abs(n - s) < 10 || t === window && !this.isWindowScrollable())
      return null;
    const a = this.getViewportHeight(t), c = this.getScrollHeight(t), l = this.getScrollDirection(n, s), u = this.calculateScrollDepth(n, c, a);
    ...
    try {
      ...
      if (s instanceof DOMException && s.name === "QuotaExceededError")
        if (this.hasQuotaExceededError = !0, o("warn", "localStorage quota exceeded, attempting cleanup", {
          ...
          try {
            if (this.storage) {
              this.storage.setItem(e, t), o("info", "Successfully stored item after cleanup");
              return;
            }
          } catch (i) {
  ...
  cleanupOldData() {
    ...
    try {
      ...
      for (let n = 0; n < this.storage.length; n++) {
        const i = this.storage.key(n);
        i?.startsWith("tracelog_") && (e.push(i), i.startsWith("tracelog_persisted_events_") && t.push(i));
      }
      if (t.length > 0)
        return t.forEach((n) => {
          try {
        ...
        }), o("info", "Cleaned up old persisted events", {
          data: { removedKeys: t.length }
        }), !0;
      const s = e.filter(
        (n) => !n.includes("session") && !n.includes("user") && !n.includes("device")
      );
      if (s.length > 0) {
        const n = s.slice(0, 5);
        return n.forEach((i) => {
        ...
        }), o("info", "Cleaned up non-critical TraceLog data", {
          data: { removedKeys: n.length }
        }), !0;
      }
      return !1;
    } catch (e) {
...
class Et extends f {
  ...
  async initWebVitals() {
    try {
      const { onLCP: e, onCLS: t, onFCP: s, onTTFB: n, onINP: i } = await Promise.resolve().then(() => Ut), a = (c) => (l) => {
    ...
    this.safeObserve(
      ...
      (e) => {
        ...
        for (const s of t) {
          ...
          i - this.lastLongTaskSentAt >= Ge && (this.shouldSendVital("LONG_TASK", n) && this.trackWebVital("LONG_TASK", n), this.lastLongTaskSentAt = i);
        }
      },
...
class mt extends f {
  ...
  shouldSuppressError(e, t) {
    ...
    return i && s - i < ue ? (this.recentErrors.set(n, s), !0) : (this.recentErrors.set(n, s), this.recentErrors.size > Qe ? (this.recentErrors.clear(), this.recentErrors.set(n, s), !1) : (this.recentErrors.size > k && this.pruneOldErrors(), !1));
  }
  pruneOldErrors() {
    const e = Date.now();
...
class pt extends f {
  ...
  async init(e) {
    if (!this.isInitialized) {
      this.managers.storage = new St();
      try {
        this.setupState(e), await this.setupIntegrations(), this.managers.event = new ot(this.managers.storage, this.integrations.googleAnalytics, this.emitter), await this.initializeHandlers(), await this.managers.event.recoverPersistedEvents().catch((t) => {
  ...
  sendCustomEvent(e, t) {
    if (!this.managers.event)
      return;
    const { valid: s, error: n, sanitizedMetadata: i } = nt(e, t);
    if (!s) {
      if (this.get("mode") === R.QA)
  ...
  setupState(e) {
    ...
    this.set("pageUrl", i);
    const a = $e() ? R.QA : void 0;
    a && this.set("mode", a);
  }
  async setupIntegrations() {
    if (this.get("config").integrations?.googleAnalytics?.measurementId?.trim())
      try {
        this.integrations.googleAnalytics = new gt(), await this.integrations.googleAnalytics.initialize();
      } catch {
        this.integrations.googleAnalytics = void 0;
      }
  }
  async initializeHandlers() {
    this.handlers.session = new ut(
...
const _t = async (r) => {
  ...
  if (!window.__traceLogDisabled && !h && !M) {
    M = !0;
    try {
      const e = Je(r), t = new pt();
      try {
        v.forEach(({ event: i, callback: a }) => {
          t.on(i, a);
        }), v.length = 0;
  ...
  U = !0;
  try {
    await h.destroy(), h = null, M = !1, v.length = 0;
  } catch (r) {
    const e = r instanceof Error && (r.message.includes("network") || r.message.includes("timeout") || r.message.includes("abort"));
    if (h = null, M = !1, v.length = 0, e)
      throw o("error", "Critical error during destroy, forced cleanup", { error: r }), r;
    o("warn", "Non-critical error during destroy, cleanup completed", { error: r });
  } finally {
    U = !1;
  }
}, xt = {
...
var q, ve = -1, N = function(r) {
  ...
  var n, i;
  return function(a) {
    e.value >= 0 && (a || s) && ((i = e.value - (n || 0)) || n === void 0) && (n = e.value, e.delta = i, e.rating = function(c, l) {
  ...
  document.visibilityState === "hidden" && w > -1 && (w = r.type === "visibilitychange" ? r.timeStamp : 0, wt());
}, me = function() {
  ...
  e = e || {}, G(function() {
    var t, s = Ie(), n = S("FCP"), i = b("paint", function(a) {
      a.forEach(function(c) {
        c.name === "first-contentful-paint" && (i.disconnect(), c.startTime < s.firstHiddenTime && (n.value = Math.max(c.startTime - V(), 0), n.entries.push(c), t(!0)));
      });
    });
    i && (t = E(r, n, Z, e.reportAllChanges), N(function(a) {
      n = S("FCP"), t = E(r, n, Z, e.reportAllChanges), ne(function() {
        n.value = performance.now() - a.timeStamp, t(!0);
      });
    }));
  });
}, J = [0.1, 0.25], Mt = function(r, e) {
  e = e || {}, ye(ie(function() {
    var t, s = S("CLS", 0), n = 0, i = [], a = function(l) {
      l.forEach(function(u) {
        if (!u.hadRecentInput) {
          var p = i[0], I = i[i.length - 1];
          n && u.startTime - I.startTime < 1e3 && u.startTime - p.startTime < 5e3 ? (n += u.value, i.push(u)) : (n = u.value, i = [u]);
        }
      }), n > s.value && (s.value = n, s.entries = i, t());
    }, c = b("layout-shift", a);
    c && (t = E(r, s, J, e.reportAllChanges), F(function() {
      a(c.takeRecords()), t(!0);
    }), N(function() {
  ...
  "interactionCount" in performance || q || (q = b("event", Nt, { type: "event", buffered: !0, durationThreshold: 0 }));
}, g = [], H = /* @__PURE__ */ new Map(), Me = 0, Rt = function() {
  var r = Math.min(g.length - 1, Math.floor((we() - Me) / 50));
  return g[r];
}, Ct = [], bt = function(r) {
  if (Ct.forEach(function(n) {
    return n(r);
...
}, ee = [200, 500], Ot = function(r, e) {
  "PerformanceEventTiming" in self && "interactionId" in PerformanceEventTiming.prototype && (e = e || {}, G(function() {
    var t;
    Lt();
    var s, n = S("INP"), i = function(c) {
      Ne(function() {
        c.forEach(bt);
        var l = Rt();
        l && l.latency !== n.value && (n.value = l.latency, n.entries = l.entries, s());
      });
...
}, te = [2500, 4e3], j = {}, Pt = function(r, e) {
  e = e || {}, G(function() {
    var t, s = Ie(), n = S("LCP"), i = function(l) {
  ...
  Dt(function() {
    var n = se();
    n && (t.value = Math.max(n.responseStart - V(), 0), t.entries = [n], s(!0), N(function() {
...
export {
  Xt as ANALYTICS_QUERY_LIMITS,
  Wt as ANOMALY_DETECTION,
  A as AppConfigValidationError,
  zt as CONTENT_ANALYTICS,
  Vt as DATA_PROTECTION,
  Qt as DEVICE_ANALYTICS,
  _ as DeviceType,
  Ft as ENGAGEMENT_THRESHOLDS,
  X as EmitterEvent,
  L as ErrorType,
  d as EventType,
  $t as INSIGHT_THRESHOLDS,
  Ht as InitializationTimeoutError,
  y as IntegrationValidationError,
  R as Mode,
  xt as PERFORMANCE_CONFIG,
  Bt as SEGMENTATION_ANALYTICS,
  Gt as SESSION_ANALYTICS,
  Yt as SPECIAL_PAGE_URLS,
