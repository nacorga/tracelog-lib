...
var SpecialProjectIds = /* @__PURE__ */ ((SpecialProjectIds2) => {
  SpecialProjectIds2["Debug"] = "debug";
  SpecialProjectIds2["Playground"] = "playground";
  return SpecialProjectIds2;
})(SpecialProjectIds || {});
...
class DebugLogger extends StateManager {
   ...
   */
  handleModeBasedEvent(level, namespace, message, data) {
    const mode = this.getCurrentMode();
    if (!mode) {
      return;
    }
  ...
  shouldUseDispatchEvent(mode) {
    ...
    if (mode === "debug") {
      try {
        ...
        return projectId === SpecialProjectIds.Debug;
      } catch {
        return false;
      }
    }
  ...
  dispatchEvent(level, namespace, message, data) {
    ...
    try {
      const event2 = new CustomEvent("tracelog:qa", {
        detail: {
...
const EVENT_SENT_INTERVAL_TEST_MS = 2500;
...
const API_BASE_URL = "https://api.tracelog.io";
...
class ConfigManager {
  async get(apiUrl, appConfig) {
    if (Object.values(SpecialProjectIds).includes(appConfig.id)) {
    ...
    const config = await this.load(apiUrl, appConfig, appConfig.id === SpecialProjectIds.Playground);
  ...
  async load(apiUrl, appConfig, isPlaygroundId) {
    try {
      const configUrl = isPlaygroundId ? `${window.location.origin}/config` : this.getUrl(apiUrl);
      if (!configUrl) {
      ...
      const errorSampling = mergedConfig.mode === "qa" || mergedConfig.mode === "debug" ? 1 : mergedConfig.errorSampling ?? 0.1;
  ...
  getDefaultConfig(appConfig) {
    ...
    const configRecords = {
      [SpecialProjectIds.Debug]: { ...defaultConfig, mode: "debug" },
      [SpecialProjectIds.Playground]: { ...defaultConfig, mode: "qa" }
    };
...
class SenderManager extends StateManager {
  ...
  prepareRequest(body) {
    const isPlaygroundId = this.get("config").id === SpecialProjectIds.Playground;
    const baseUrl = isPlaygroundId ? window.location.origin : this.get("apiUrl") ?? API_BASE_URL;
    return {
      url: `${baseUrl}/events`,
  ...
  shouldSkipSend() {
    return this.get("config").id === SpecialProjectIds.Debug;
  }
...
class EventManager extends StateManager {
  ...
  get clearQueueIntervalInstance() {
    return () => {
    };
  }
...
class NetworkHandler extends StateManager {
  ...
  interceptFetch() {
    window.fetch = async (input, init2) => {
      ...
      try {
        const response = await this.originalFetch(input, init2);
